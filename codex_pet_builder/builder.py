...
def load_manifest(root: Path) -> dict[str, object]:
    ...
    if manifest.get("petId") != pet.get("id"):
        raise ValueError("source/generation-manifest.json: petId must match pet.json id")
    return manifest
...
def build(root: Path, output_root: Path | None = None) -> Path:
    ...
    key = _parse_key(manifest["chromaKey"])
    threshold = float(manifest["chromaThreshold"])
