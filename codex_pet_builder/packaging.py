...
def check_release(root: Path, output_dir: Path | None = None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="codex-pet-release-") as temporary:
        ...
        expected = decoded_pixel_hash(root / pet["spritesheetPath"])
    ...
    evidence_kind = find_qa_evidence(root, expected)["kind"]
    print(f"release check passed; QA evidence kind {evidence_kind}; decoded RGBA SHA-256 {expected}")
