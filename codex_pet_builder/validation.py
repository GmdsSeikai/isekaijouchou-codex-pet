...
from .project import find_qa_evidence, project_qa_paths
...
def _strict_qa_errors(root: Path, spritesheet: Path) -> list[str]:
    ...
    evidence_index, summary = project_qa_paths(root)
    ...
    try:
        ...
        return [*errors, f"{evidence_index.name}: {error}"]
    if evidence is None:
        return [*errors, f"{evidence_index.name}: no audited evidence for decoded spritesheet SHA-256 {digest}"]
    evidence_path = evidence.get("path")
    if not isinstance(evidence_path, str):
        return [*errors, f"{evidence_index.name}: matched evidence must include a relative path"]
    candidate = Path(evidence_path)
    if candidate.is_absolute() or ".." in candidate.parts:
        return [*errors, f"{evidence_index.name}: evidence path must be repository-relative and cannot escape the repository"]
    qa_dir = (root / candidate).resolve()
    try:
        qa_dir.relative_to(root.resolve())
    except ValueError:
        return [*errors, f"{evidence_index.name}: evidence path escapes the repository"]
    kind = evidence.get("kind")
    if kind == "synthetic-fixture":
        if any(key in evidence for key in ("reviewedRelease", "reviewerCount", "classificationCount", "reviewStatus")):
            return [*errors, "synthetic fixture evidence must not claim human review"]
        report_path = qa_dir / "synthetic-validation.json"
        if not report_path.is_file():
            return [*errors, f"missing synthetic fixture report: {report_path.relative_to(root).as_posix()}"]
        report = json.loads(report_path.read_text(encoding="utf-8"))
        if report != {"kind": "synthetic-fixture", "ok": True, "spritesheetSha256": digest}:
            errors.append("synthetic fixture report is stale or invalid")
        return errors
    if kind != "human-visual":
        return [*errors, f"{evidence_index.name}: unsupported evidence kind: {kind}"]
    source_release = evidence.get("reviewedRelease")
    if not isinstance(source_release, str):
        errors.append(f"{evidence_index.name}: matched evidence has invalid release provenance")
    elif candidate.name != source_release:
        errors.append(f"{evidence_index.name}: evidence directory does not match its declared reviewedRelease")
    required = (
    ...
    if evidence.get("reviewerCount") != derived_reviewer_count:
        errors.append(f"{evidence_index.name}: reviewerCount does not match the reviewer payloads")
    if evidence.get("classificationCount") != derived_classification_count:
        errors.append(f"{evidence_index.name}: classificationCount does not match the answer-key classifications")
    if evidence.get("reviewStatus") != derived_status:
        errors.append(f"{evidence_index.name}: reviewStatus does not match the computed reviewer result")
    if evidence.get("warnings") != computed.get("warnings") or evidence.get("unconfirmed") != computed.get("unconfirmed"):
        errors.append(f"{evidence_index.name}: warnings or unconfirmed results do not match the computed reviewer result")
    return errors
...
def validate_repository(
    ...
    if strict:
        result["qaEvidenceKind"] = find_qa_evidence(root, decoded_pixel_hash(spritesheet))["kind"]
